...
def inception_resnet_v2(
  ...
  Args:
    ...
    from_scratch: Whether we are training the inception resnet from scratch on
      imagenet, or we are just finetuning on some other dataset. If we are
      just finetuning, then set the batch normalization layers to evaluation
      mode.
  ...
  with tf.variable_scope(
      scope, 'InceptionResnetV2', [inputs, num_classes], reuse=reuse) as scope:
    with slim.arg_scope(
        [slim.batch_norm], is_training=is_training and from_scratch):
      with slim.arg_scope([slim.dropout], is_training=is_training):
        ...
        with tf.variable_scope('Logits'):
          ...
          # TODO(vrama): remove hardcoding
          logit_mlp = slim.fully_connected(
              net, logit_mlp_dim, activation_fn=tf.nn.relu, scope='PreLogits')
          logits = slim.fully_connected(
              logit_mlp, num_classes, activation_fn=None, scope='Logits')

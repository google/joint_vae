...
from joint_vae.encoder_decoder import NIPS17MnistaProductOfExpertsEncoder
from joint_vae.encoder_decoder import NIPS17MultiLayerPerceptronLabelEncoder
from joint_vae.encoder_decoder import NIPS17MnistaMultiLayerPerceptronLabelEncoder
from joint_vae.encoder_decoder import NIPS17MnistaCategoricalLabelDecoder
...
from joint_vae.encoder_decoder import NIPS17MnistaMultiLayerPerceptronMultiModalEncoder
...
def get_nips_17_conv_multi_vae_networks(latent_dim,
  ...
  tf.logging.info('Swap out mlp relu for elu set to : %s' % (swap_out_mlp_relu_for_elu))
  # Build the image encoder and prior (the only ones that actually matches
  # the convolutional multi VAE architecture).
  def build_x_encoder(name, last_layer_size, outputs_per_dim, dont_project=False):
  ...
  def build_y_encoder(name, last_layer_size, outputs_per_dim, dont_project=False):
    # TODO(vrama): Complete the details of this encoder.
    y_encoder_module = (NIPS17MnistaProductOfExpertsEncoder if product_of_experts_y else
                        NIPS17MnistaMultiLayerPerceptronLabelEncoder)
  ...
  tf.logging.info('Using mnista encoder decoders.')
  ...
  xy_encoder = NIPS17MnistaMultiLayerPerceptronMultiModalEncoder(
      [
          build_x_encoder('inference_x_xy', mlp_dim, outputs_per_dim=1, dont_project=True),
          build_y_encoder('inference_y_xy', mlp_dim, outputs_per_dim=1, dont_project=True)
      ],
      mlp_dim,
      latent_dim,
    swap_out_mlp_relu_for_elu=swap_out_mlp_relu_for_elu,
  ...
  if FLAGS.image_decoder_channels == '_REPLACE_ME':
    ...
    y_decoder = NIPS17MnistaCategoricalLabelDecoder(
      mlp_dim=128,
      data_dims=vocab_sizes,
      num_layers_mlp=2,
      l1_pyz=l1_pyz,

...
"""A minimal interface mlp module, with dropout."""
# TODO(hmoraldo): get rid of this, and instead use a dropout activation
# function over vanilla MLP. See cl/161960594 for details.
...
class MLP(base.AbstractModule, base.Transposable):
  """A Multi-Layer perceptron module with dropout."""
  ...
  def __init__(self,
               ...
               keep_prob=0.9,
               name="mlp"):
    """Constructs an MLP module with dropout.
    ...
    Args:
      ...
      use_bias: Whether to include bias parameters in the linear layers.
        Default `True`.
      dropout: Optional Bool which applies dropout after each activation layer
        if
        set to true.
      keep_prob: float, indicates with what probability to keep the activation's
        value for dropout. Only applies if dropout is True.
    ...
    with self._enter_variable_scope():
      self._layers = [
          basic.Linear(
              ...
              use_bias=self.use_bias) for i in xrange(self._num_layers)
      ]
  ...
  def _build(self, inputs, is_training=True):
    ...
    Args:
      ...
      is_training: Boolean, True if the model is training.
      ...
      if final_index != layer_id or self._activate_final:
        ...
        if self._dropout:
          net = tf.contrib.layers.dropout(
              net, keep_prob=self._keep_prob, is_training=is_training)
